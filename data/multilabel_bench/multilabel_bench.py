...
MAIN_PATH = 'https://huggingface.co/datasets/kiddothe2b/multilabel_bench/resolve/main'
...
class MultiLabelBench(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        MultiLabelBenchConfig(
            ...
            dev_column="dev",
            url='TBA',
            data_url=f"{MAIN_PATH}/mimic.tar.gz",
        ...
        MultiLabelBenchConfig(
            ...
            dev_column="dev",
            url='TBA',
            data_url=f"{MAIN_PATH}/bioasq.tar.gz",
        ...
        MultiLabelBenchConfig(
            ...
            dev_column="dev",
            url='TBA',
            data_url=f"{MAIN_PATH}/eurlex.tar.gz",
        ...
        MultiLabelBenchConfig(
            ...
            dev_column="dev",
            url='TBA',
            data_url=f"{MAIN_PATH}/ecthr.tar.gz",
    ...
    def _info(self):
        features = {"doc_id": datasets.Value("string"),
                    "text": datasets.Value("string"),
                    "labels": datasets.features.Sequence(datasets.ClassLabel(names=self.config.label_classes))}
    ...
    def _split_generators(self, dl_manager):
        data_dir = dl_manager.download_and_extract(os.path.join(self.config.data_url))
        return [
        ...
        with open(filepath, "r", encoding="utf-8") as f:
            for id_, row in enumerate(f):
                ...
                if data["data_type"] == split:
                    if 'ecthr' in self.config.name:
                        labels = [label for label in sorted(data[self.config.label_column])
                                  if label in ECTHR_ARTICLES[self.config.label_level]]
                        yield id_, {
                            "doc_id": data[self.config.id_column],
                            "text": ' '.join(data[self.config.text_column]),
                        ...
                        yield id_, {
                            ...
                            "labels": sorted(data[self.config.label_column][self.config.label_level]),
                        }
