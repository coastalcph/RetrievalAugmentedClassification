#!/bin/bash
#SBATCH --job-name=bioasq-ra-labels-neighbors-pubmedbert-base
#SBATCH --cpus-per-task=8 --mem=8000M
#SBATCH -p gpu --gres=gpu:a100:1
#SBATCH --output=/home/rwg642/RetrievalAugmentedClassification/bioasq-ra-labels-neighbors-pubmedbert-base.txt
...
for NO_NEIGHBORS in 64 128 256
do
  ...
  python classifier/train_classifier.py \
      ...
      --augment_with_documents true \
      --augment_with_labels false \
      --encode_document true \
      --no_neighbors ${NO_NEIGHBORS} \
      --dataset_name ${DATASET_NAME} \
      --output_dir data/${DATASET_NAME}/${MODEL_PATH}-ra/nn-${NO_NEIGHBORS}-dl-1-ah-1 \
      --do_train \
      --do_eval \
      --do_pred \
